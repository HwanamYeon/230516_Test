        ...
        <v-card-text>
            <v-row no-gutters style="margin: 15px 0px -30px -15px;">
                <v-col style="max-width:140px;">
                    <String class="attributes-list" label="FindByName" v-model="value.parameters.findByName" :editMode="editMode"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        ...
        created() {
            this.value.parameters.findByName = '';
        },
        watch: {
        },
        methods: {
            search() {
                let search = null;
                if(this.value.parameters.findByName == '' ) {
                    search = null;
                }else{
                    search = this.value;
                }
